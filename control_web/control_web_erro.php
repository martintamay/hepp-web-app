...
$old_error_handler = set_error_handler("control_web_scriptcase_error_handler");

function control_web_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['control_web']['contr_erro']) && $_SESSION['scriptcase']['control_web']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_control_web->contr_control_web->Apl_com_erro = true;
            if ($inicial_control_web->contr_control_web->NM_ajax_flag)
            {
                $inicial_control_web->contr_control_web->NM_ajax_info['errList']['geral_control_web'][] = $msg;
                return;
            }
            control_web_scriptcase_error_display($msg, $err_no);
        }
...
function control_web_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['control_web'])  && '' != $_SESSION['scriptcase']['error_icon']['control_web'])  ? $_SESSION['scriptcase']['error_icon']['control_web']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['control_web']) && '' != $_SESSION['scriptcase']['error_close']['control_web']) ? $_SESSION['scriptcase']['error_close']['control_web'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['control_web']) && '' != $_SESSION['scriptcase']['error_span_title']['control_web']) ? $_SESSION['scriptcase']['error_span_title']['control_web'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['control_web']) && '' != $_SESSION['scriptcase']['error_icon_title']['control_web']) ? $_SESSION['scriptcase']['error_icon_title']['control_web'] : '';
?>
